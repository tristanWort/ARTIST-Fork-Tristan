...
def build_heliostat_file_list(central_data_dir: str, heliostat_name=None):
    """
    ...
    for heliostat_dir in sorted(base_path.iterdir()):
        ...
        elif heliostat_name is not None:
            if heliostat_id != heliostat_name:
                continue 
...
if __name__ == "__main__":
    ...
    heliostats = ["AO51", "AE32", "AP30", "AP41", "AH30", "AF46"]
    
    for heliostat in heliostats:
        scenario_name = heliostat

        # A tuple of: (helisotat-name, heliostat-properties.json, deflectometry.h5)
        heliostat_files_list = build_heliostat_file_list(f"/dss/dsshome1/05/di38kid/data/paint/selected_20", heliostat_name=heliostat)
