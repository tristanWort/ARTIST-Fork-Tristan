...
save_dir = Path("/dss/dsshome1/05/di38kid/data/simulated_data/SPIE_PAPER/blocking_effect/BB72")
...
for blocking in blockings:
    ...
    for n_sample, data in enumerate(batch[:1]):
        ...
        for h, heliostat_id in enumerate(all_heliostat_ids):
            ...
            sigma_gaussian_blocking = t.item() / 5
            print(t.item(), sigma_gaussian_blocking) 
            bitmap_blocked = apply_artificial_blocking(bitmap, block_ratio=blocking / 100, threshold=0.1, sigma=sigma_gaussian_blocking)
            ...
            if blocking != 0:   
                ...
                save_bitmap_with_center_crosses(bitmap_blocked, [blocked_indices, ideal_indices], save_dir / f'{heliostat_id}_error_of_{blocking}_blocking__raw.png', 
