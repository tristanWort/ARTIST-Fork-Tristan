...
class CalibrationModel(nn.Module):
    ...
    """
    def __init__(self, run_config: Dict[str, Dict], name: str):
        """
    ...
    def evaluate_model(self, epoch: int, orientations: torch.Tensor, pred_flux_bitmaps: torch.Tensor, field_batch: List[Dict]):
        ...
        return alignment_loss, eval_alignment_errors, actual_alignment_errors
    ...
    def calc_contour_loss(pred_bitmaps: torch.Tensor, true_bitmaps: torch.Tensor, 
        ...
        fine_loss_per_heliostat = torch.empty(H, device=pred_bitmaps.device)
        coarse_loss_per_heliostat = torch.empty(H, device=pred_bitmaps.device)
        pred_contours = torch.empty_like(pred_bitmaps)
        true_contours = torch.empty_like(pred_bitmaps)
        
        # Use mse_function for pixel-wise image loss
        mse_fnc = torch.nn.MSELoss()
        
        # Iterate over Heliostat indices
        for h_idx in range(H):
            pred_contours[:, H] = self.find_upper_contour(pred_bitmaps[:, H], threshold, sharpness, num_interpolate, sigma_in, sigma_out)
            true_contours[:, H] = self.find_upper_contour(true_bitmaps[:, H], threshold, sharpness, num_interpolate, sigma_in, sigma_out)
            fine_loss_per_heliostat[H] = mse_fnc(pred_contours[:, H], true_contours[:, H])
        
        # Get the contour loss configurations
        loss_config = self.run_config[my_config_dict.run_config_general][my_config_dict.general_loss_config][my_config_dict.general_contour_config]
        coarse_alignment_with = loss_config[my_config_dict.general_contour_coarse]
