...
class TensorboardLogger:
    ...
    def log_alignment_errors(self, epoch: int, alignment_errors: torch.Tensor, helio_and_calib_ids=None):
        """
        ...
        alignment_errors : torch.Tensor
            Tensor of shape [B, H] or [H, B] in mrad.
        helio_and_calib_ids : Dict[str, List[int]]
            Mapping from heliostat ID to list of calibration IDs (default is None, then use self.helio_and_calib_ids).
        """
        ...
        # Summary metrics
        self.log_metric(epoch, "AlignmentErrors_mrad/Avg", alignment_errors.mean().item())
        self.log_metric(epoch, "AlignmentErrors_mrad/Med", alignment_errors.median().item())
        ...
        for h_idx, (helio_id, calib_ids) in enumerate(helio_and_calib_ids.items()):
            ...
            self.log_heliostat_metric(epoch, "AlignmentErrors_mrad/Avg", helio_id, helio_errors.mean().item())
            self.log_heliostat_metric(epoch, "AlignmentErrors_mrad/Med", helio_id, helio_errors.median().item())

            for c_idx, (calib_id, err) in enumerate(zip(calib_ids, helio_errors.tolist())):
                self.log_heliostat_metric(epoch, "AlignmentErrors_mrad", helio_id, err, calib_id=calib_id)
