...
df = pd.read_csv(heliostats_deflectometry_metadata)
...
og_df = df.copy()
df = df[df["HeliostatId"].isin(sufficient_heliostats)].reset_index(drop=True)
...
df['cluster'] = kmeans.fit_predict(df[['east', 'north']])

# === Select one representative heliostat per cluster ===
selected_ids = df.groupby('cluster').apply(lambda g: g.sample(1, random_state=42)).reset_index(drop=True)
selected_ids_list = selected_ids['HeliostatId'].tolist()

selected_df = df[df['HeliostatId'].isin(selected_ids_list)].copy()
full_df = df.copy()
...
selected_df, replaced_pairs = enforce_heliostats_in_selection(df, selected_df, target_ids)
...
ax2.scatter(og_df['east'], og_df['north'], color='lightgray', s=20, label='All Heliostats')
