...
class TensorboardLogger:
    ...
    def __init__(self, name: str, log_dir: str = None, **kwargs):
        """
        ...
        log_dir : str, optional
            Directory where logs will be saved. If None, defaults to 'runs/{name}/log'
        **kwargs
    ...
    def log_metric(self, metric_name: str, value: float, epoch: int):
        """
        Log a metric value.
    ...
    def log_heliostat_metric(self, metric_name: str, heliostat_id: str, 
                             value: float, epoch: int, calib_id: int = None):
        """
        ...
        if self.mode != "None":
            if calib_id is not None:
                ...
                self.writer.add_scalar(f"{metric_name}/Avg/{self.mode}/{heliostat_id}", value, epoch)
    
    def log_parameter_as_scalar(self, param_name: str, heliostat_id: str, 
                               value: torch.Tensor, epoch: int, index: int = None):
        """
        ...
        if self.mode != "None":
            if index is not None:
                self.writer.add_scalar(f"Parameters/{param_name}/{heliostat_id}/{index}", 
                                    value.item(), epoch)
            else:
                self.writer.add_scalar(f"Parameters/{param_name}/{heliostat_id}", 
                                    value.item(), epoch)
    
    def log_parameter_as_histogram(self, param_name: str, heliostat_id: str, 
                                  value: torch.Tensor, epoch: int):
        """
        ...
        """
        if self.mode != "None":
            self.writer.add_histogram(f"Parameters/{param_name}/{heliostat_id}", 
                                    value.cpu().detach().numpy(), epoch)
    
    def log_parameters(self, params_dict: Dict[str, torch.nn.Parameter], 
                      heliostat_names: List[str], epoch: int, use_scalars: bool = True):
        """
        ...
        if self.mode != None:
            for param_name, param in params_dict.items():
                if isinstance(param, (list, torch.nn.ParameterList)) and len(param) == len(heliostat_names):
                    for h_idx, (h_name, param_value) in enumerate(zip(heliostat_names, param)):
                        if use_scalars:
                            if param_value.numel() == 1:
                                self.log_parameter_as_scalar(param_name, h_name, param_value, epoch)
                            else:
                                # For multi-element parameters, log each element as a separate scalar
                                for i, val in enumerate(param_value.flatten()):
                                    self.log_parameter_as_scalar(param_name, h_name, val, epoch, index=i)
                        else:
                            self.log_parameter_as_histogram(param_name, h_name, param_value, epoch)
    
    def log_image(self, image_name: str, heliostat_id: str, image: torch.Tensor,
                  epoch: int, calib_id: int = None):
        """
        Log an image.
        ...
        if self.mode != "None":
            if calib_id is not None:
                self.writer.add_image(f"{image_name}/{self.mode}/{heliostat_id}/{calib_id}", 
                                    image.cpu().detach(), epoch, dataformats='HW')
            else:
                self.writer.add_image(f"{image_name}/{self.mode}/{heliostat_id}", 
                                    image.cpu().detach(), epoch, dataformats='HW')
    
    def close(self):
        """Close the SummaryWriter."""
        self.writer.close()
        log.info(f"TensorboardLogger closed: {self.name}")
